...
export const ToolCallCollapsed = ({ event, onEnterSubagent }: Props) => {
  ...
  return (
    <div
      className={cn(
        "tool-call collapsed relative border border-border rounded my-2",
    ...
    >
      ...
      {agentId && onEnterSubagent && (
        <button
          type="button"
          onClick={() => onEnterSubagent(agentId, event.event_uuid)}
